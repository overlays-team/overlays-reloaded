        ...
        if (isEnabled)
        {
            ...
            switch (playerMode)
            {
                case PlayerMode.Default:
                    ...
                    if (Input.GetMouseButtonDown(0) && Input.touchCount <= 1 || Input.GetMouseButtonDown(2)) //wir setzen touch count auf max 1, weil sich unser programm sonst nicht entscheiden kann zwischen 2 Fingern
                    {
                        Debug.Log("one click");
